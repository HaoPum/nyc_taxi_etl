...
def process_weather_data(spark, input_path):
    ...
    for record in weather_data:
        processed_record = {
            ...
            "latitute": str(record.get("latitute", "")),
            "longitude": float(record.get("longitude", 0.0)),
            "temperature_celsius": float(record.get("temperature_celsius", 0.0)),
            "temperature_fahrenheit": float(record.get("temperature_fahrenheit", 0.0)),
            "humidity_percent": float(record.get("humidity_percent", 0.0)),
            "pressure_hpa": float(record.get("pressure_hpa", 0.0)),
            "wind_speed_kmh": float(record.get("wind_speed_kmh", 0.0)),
            "weather_condition": str(record.get("weather_condition", "")),
            "visibility_km": float(record.get("visibility_km", 0.0)),
            "uv_index": float(record.get("uv_index", 0.0)),
    ...
    df = spark.createDataFrame(processed_data, schema=weather_schema)
